...
| isDangerous   | bool                                            | ☐        | false                                                        | Refer to `isDangerous` under [Suggestion Object](#suggestion-object) |
| isOptional    | bool                                            | ☐        | false                                                        | True if a CLI command is optional                            |
...
| variadic      | boolean                                         | ☐        | false                                                        | Specifies that the argument is variadic and therefore repeats infinitely. e.g. `echo` takes a variadic argument (`echo hello world ...`) |
...
| Property Name | Type | Required | Default | Description |
|---|---|---|---|---
| name | string | ☐ |  |  The name of an argument. This is a normal string that signals to the user the type of argument they are inserting if there are no available suggestions.  Unlike the subcommand and option objects, Fig does not use this value for parsing. Therefore, it can be whatever you want.    For git commit -m, it takes an option with the name "message" |  |
| description | string | ☐ |  |  The text that gets rendered at the bottom of the autocomplete box.  Keep it short and direct! |  |
| isDangerous | boolean | ☐ |  |  Specifies whether the suggestion is "dangerous". If so, Fig will not enable  its insert and run functionality whereby selecting a suggestion runs a command.  This will make it harder for a user to accidentally run a dangerous command.    This is used in specs like rm and trash. |  |
